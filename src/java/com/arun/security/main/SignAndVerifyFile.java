...
import static com.arun.security.main.Constants.INPUT_FILE;
...
public class SignAndVerifyFile {
	public static void main(String[] args) throws UnrecoverableKeyException,
		...
		File dataToSign = new File(BASE_FOLDER, INPUT_FILE);
		...
		byte[] realSig = sign(dataToSign, keyStoreFile, storePassword, alias,
				keyPassword, signAlgorithm);

		writeToFile(signatureFile, realSig);

		System.out.println("signature verifies: "
				+ verify(certificateFile, signatureFile, dataToSign,
						signAlgorithm));
	}
}
